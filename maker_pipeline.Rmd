...
cmd    <- with(metadataAssemblies,
               paste(# Create the tbl2asn dir:
                     ...
                     # Prepare the genome.tbl for tbl2asn:
                     "cd ", paste(postMaker, "gag/", previousGagDir, sep = ""), " && ",
                     "mv genome.tbl genome.tbl.bak", "; ",
                     
                     # Prepare the genome.tbl for tbl2asn, sed:
                     "cd ", paste(postMaker, "gag/", previousGagDir, sep = ""), " && ",
...
# ** Note - before running, edit the bash .sub files to fix the portion for sed, which doesn't turn out properly.
```
...
$ cp Lachnellula_arida/Lari_RepeatDB/postMakerProcessing/gag/gag_pass2_eval/genome.proteins.fasta Lachnellula_arida/Lari_RepeatDB/postMakerProcessing/gag/gag_pass2_eval/Lari.proteins.gagPass2.fasta  
  
Make changes to the proteins fasta to look like the original Maker output:  
$ sed -i 's/>protein|/>/g' Lachnellula_arida/Lari_RepeatDB/postMakerProcessing/gag/gag_pass2_eval/Lari.proteins.gagPass2.fasta  
$ sed -i 's/ ID=.*$/ protein/g' Lachnellula_arida/Lari_RepeatDB/postMakerProcessing/gag/gag_pass2_eval/Lari.proteins.gagPass2.fasta  
Take a look:  
...
Make a copy, and remove the * at the end of the protein sequences for IPR scan.  
$ cp Lari.proteins.gagPass2.fasta Lari.proteins.gagPass2.nostop4ipr.fasta  
$ sed -i 's/*//g' Lari.proteins.gagPass2.nostop4ipr.fasta  
Run IPR scan with lookup service, with only pfam.  
$ ~/prog/my_interproscan/interproscan-5.24-63.0/interproscan.sh -appl pfam -f TSV -iprlookup -goterms --cpu 11 -pa -t p -i Lari.proteins.gagPass2.nostop4ipr.fasta -o output.iprscan  

Example one-liner:  
$ cp Lsue.proteins.gagPass2.fasta Lsue.proteins.gagPass2.nostop4ipr.fasta; sed -i 's/*//g' Lsue.proteins.gagPass2.nostop4ipr.fasta; head -5 Lsue.proteins.gagPass2.nostop4ipr.fasta;  ~/prog/my_interproscan/interproscan-5.24-63.0/interproscan.sh -appl pfam -f TSV -iprlookup -goterms --cpu 23 -pa -t p -i Lsue.proteins.gagPass2.nostop4ipr.fasta -o output.iprscan
...
cmd <- with(metadataAssemblies, paste("cd ", postMaker, " && ", anniePath, " -b ", SppAbbrv, "_uniProtswissProt_blast.out",
                                      " -g ", SppAbbrv, "_gag_edited.renamed.gff", " -db ", uniProtSwissPDB, sep = ""))
cmd[1]
sapply(cmd, function(x) system(x))
```

*** fsa mentioned here, could filter contigs less than 200 nt before this?  
...
cmd <- with(metadataAssemblies, paste("cd ", postMaker, " && ", gagPath, " -f ", SppAbbrv, "_gag_edited.fsa",
                                      " -g ", SppAbbrv, "_gag_edited.renamed.gff"," -a annie_output.tsv", 
