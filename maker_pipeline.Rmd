...
We conclude by assembling 
together the GFF and FASTA outputs.
```{r}
...
```{r}
cmd <- with(metadataAssemblies,
            paste("cd ", snapRnd1MakerRnd1Path, " && ", 'grep ">" genome.dna | wc -l ', sep = ""))
sapply(cmd, function(x) system(x))

cmd <- with(metadataAssemblies,
            file.rename(paste(snapRnd1MakerRnd1Path, "genome.dna", sep = ""),
                        paste(snapRnd1MakerRnd1Path, SppAbbrv, "_rnd1.zff.dna", sep = "")))
sapply(cmd, function(x) system(x))

cmd <- with(metadataAssemblies,
            file.rename(paste(snapRnd1MakerRnd1Path, "genome.ann", sep = ""),
...
file in the busco/config/ directory.
```{r}
...
cmd <- with(metadataAssemblies,
            paste("cd ", snapRnd1MakerRnd1Path, " && ",
                  ...
                  " -o ", paste(SppAbbrv, "_rnd1_maker", sep = ""),
...
# Rather than develo a script, I went to each dir and used rename on the files:
...
cmd <- with(metadataAssemblies,
            paste("cd ", snapRnd1MakerRnd2Path, " && ", maker2zffPath, 
                  # " -x 0.25 -l 50 -c 0 -e 0 -o 0 -d ", # option to filter junky models even w/o EST data avail.
                  " -n -d ", 
                  paste(makerRnd2_OutputPath, SppAbbrv, "_rnd2_master_datastore_index.log", sep = ""), sep = ""))
sapply(cmd, function(x) system(x))
```
...
cmd <- with(metadataAssemblies, file.rename(paste(snapRnd1MakerRnd2Path, "genome.dna", sep = ""),
                                            paste(snapRnd1MakerRnd2Path, SppAbbrv, "_rnd1.zff.dna", sep = "")))
sapply(cmd, function(x) system(x))

cmd <- with(metadataAssemblies, file.rename(paste(snapRnd1MakerRnd2Path, "genome.ann", sep = ""),
                                            paste(snapRnd1MakerRnd2Path, SppAbbrv, "_rnd1.zff.ann", sep = "")))
sapply(cmd, function(x) system(x))
```
...
cmd <- with(metadataAssemblies, paste("cd ", snapRnd1MakerRnd2Path, " && ", fathomPath, " ", 
                                      paste(SppAbbrv, "_rnd1.zff.ann", sep = ""), " ", paste(SppAbbrv, "_rnd1.zff.dna", sep = ""),
                                      " -gene-stats > gene-stats.log 2>&1 ", sep = ""))
sapply(cmd, function(x) system(x))
...
cmd <- with(metadataAssemblies, paste("cd ", snapRnd1MakerRnd2Path, " && ",
                  fathomPath, " uni.ann uni.dna -export 1000 -plus > uni-plus.log 2>&1 ", sep = ""))
sapply(cmd, function(x) system(x))
```
