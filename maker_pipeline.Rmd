...
uniProtSwissPDB <- "/home/CFIA-ACIA/girouxeml/prog/databases/uniprot_sprot.fasta"
...
jbrowsePrepRefsSeqsPath <- "/home/CFIA-ACIA/girouxeml/prog/jbrowse/JBrowse-1.12.3/bin/prepare-refseqs.pl"
...
```{r}
jbrowseFlatfile2json <- "/home/CFIA-ACIA/girouxeml/prog/jbrowse/JBrowse-1.12.3/bin/flatfile-to-json.pl"
...
overwrite the original one. Instead of JBrowse, I used Geneious to visualise the annotations, but going through the steps to 
prepare good GFF files for JBrowse allowed me to identify and fix problems in the GFF3 files.  
  
Record the final fixed gff3 files in the metadata table:
```{r}
metadataAssemblies$fixed_GFF3 <- paste(metadataAssemblies$makerRnd3_OutputPath, metadataAssemblies$SppAbbrv, "_rnd3.all.maker_jb.gff3", sep = "")
```
...
Map ID file:
```{r}
prefix <- "post_maker_map_IDs"
justify = 6
abrvGene = "G"
abrvTrans = "T"

cmd <- with(metadataAssemblies,
            paste("perl ", makerMapPath, " --prefix ", paste(SppAbbrv, "_", sep = ""), 
                  " --justify ", justify, " --abrv_gene ", abrvGene, " --abrv_tran ", abrvTrans, 
                  " ", fixed_GFF3, " > ", paste(makerRnd3_OutputPath, SppAbbrv, ".map", sep = "")))
...
cmd <- with(metadataAssemblies, 
            paste("cp ", fixed_GFF3, paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3", sep = "")))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)
```
...
GFF and protein fasta files:
```{r}
prefix <- "post_maker_cp_proteinsFasta"
cmd <- with(metadataAssemblies, 
            paste("cp ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.fasta", sep = ""), 
                  " ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.fasta", sep = "")))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)

prefix2 <- "post_maker_cp_transcriptsFasta"
cmd2 <- with(metadataAssemblies, 
             paste("cp ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.transcripts.fasta", sep = ""), 
                   " ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.transcripts.renamed.fasta", sep = "")))
cmd2[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd2, prefix2, suffix)
```
...
Now to rename the GFF3 files, using the ID mapping files created:
```{r}
prefix <- "post_maker_Renaming"
cmd <- with(metadataAssemblies,
            paste("perl ", makerMapGffPath, " ", paste(makerRnd3_OutputPath, SppAbbrv, ".map", sep = ""),
                  " ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3", sep = ""), sep = ""))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)                
```
...
Now to rename the protein and transcript fasta, using the ID mapping files created:
```{r}
prefix <- "post_maker_Renaming_Protein"
cmd <- with(metadataAssemblies,
            paste("perl ", makerMapFastaPath, " ", paste(makerRnd3_OutputPath, SppAbbrv, ".map", sep = ""),
                  " ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.fasta", sep = ""), sep = ""))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)

prefix2 <- "post_maker_Renaming_Transcripts"
cmd2 <- with(metadataAssemblies,
             paste("perl ", makerMapFastaPath, " ", paste(makerRnd3_OutputPath, SppAbbrv, ".map", sep = ""),
                   " ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.transcripts.renamed.fasta", sep = ""), sep = ""))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd2, prefix2, suffix) 
```
...
### Run a BLASTp of the renamed maker proteins fasta file against the uniprot-swiss-prot protein database.  
Make sure you renamed your fasta files first and that you run a protein to protein BLAST.
```{r}
prefix <- "post_maker_BLASTp_uniProt_SwissProt"
cmd <- with(metadataAssemblies,
            paste(blastallPath, " -p blastp ", 
                  " -i ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.fasta", sep = ""),
                  " -d ", uniProtSwissPDB, " -e 0.01 -m 8 -b 1 ",
                  " > ", paste(RepeatDBPath, SppAbbrv, "_uniProtswissProt_blast.out", sep = ""), sep = ""))
cmd[1]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix) 
```

### Run InterProScan on the MAKER proteins fasta files.  
Here we run our proteins fasta files against the local installation of InterProScan. Not all functionallity is available
since there are some components that require glbic updates for a couple of the database searches. The main thing here is to 
make sure the output is in the format we require for the next step in MAKER which extracts the functional annotation obtained 
from InterProScan to the annotations in our GFF3 file.
```{r}
prefix <- "post_maker_InterProScan_noGene3d"
node <- "24"
...
cmd <- with(metadataAssemblies,
            paste("cd ", RepeatDBPath, " && ", interproPath, " ",
                  " -i ", paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.fasta", sep = ""),
                  " -appl ", interproAppsNoGene3d, " --cpu 24 ", sep = ""))
cmd[3]
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix, node)
```
