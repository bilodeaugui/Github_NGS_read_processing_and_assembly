...
# Note, require entire proteome froma  min of two related species and perhaps all of UniProt/SwissProt.
...
# https://www.ncbi.nlm.nih.gov/genome/?term=txid5180[orgn]
```
...
```{r}
i    <- 1 
node <- 6 # Number of processors to use when qloged-in for this bash script

for(i in 1:nrow(metadataAssemblies)){
    cmd[i] <- paste("RepeatModeler -pa ", node,
...
```{r}
node <- 11   # Number of processors to use when qloged-in for this bash script

for(i in 1:nrow(metadataAssemblies)){
    prefix <- "embl_mask"
    cmd <- paste("RepeatMasker -pa ", node,
...
pattern <- "protein= "
newPattern <- paste("protein=", pepBotcinPath, ",", pepMarbrunPath, ",", pepSclerscPath, sep = "")
...
cmd <- with(metadataAssemblies,
            paste("cd ", snapRnd1MakerRnd1Path, " && ", maker2zffPath, " -n -d ", 
...
```{r}
for(k in 1:nrow(metadataAssemblies)){
    dir.create(paste(metadataAssemblies$maker_rnd1_path[k], "genemark-es_round1", sep = ""),
               showWarnings = TRUE, recursive = FALSE)}
metadataAssemblies$genemarkEsRnd1Path <- paste(metadataAssemblies$maker_rnd1_path, "genemark-es_round1/", sep = "")
```
