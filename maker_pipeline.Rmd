...
hmmAssemPath    <- paste(progPath, "hmm-assembler.pl", sep = "") 
...
cmd <- with(metadataAssemblies,
            paste("perl ", makerMapPath, " --prefix ", paste(SppAbbrv, "_", sep = ""), 
                  ...
                  " ", fixed_GFF3_jb, " > ", paste(postMaker, SppAbbrv, ".map", sep = "")))
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)
```
...
cmd <- with(metadataAssemblies, 
            paste("cp ", fixed_GFF3_jb, " ", paste(postMaker, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3;", sep = ""),
                  " gzip ", fixed_GFF3_jb, sep = ""))
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)
```
...
cmd <- with(metadataAssemblies,
            paste(# Renaming the GFF3 files:  
                  ...
                  " ", paste(postMaker, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3; ", sep = ""), 
...
cmd <- with(metadataAssemblies,
            paste(blastallPath, " -p blastp ", 
                  " -i ", paste(postMaker, SppAbbrv, "_rnd3.maker.proteins.renamed.fasta", sep = ""),
                  " -d ", uniProtSwissPDB, " -e 0.01 -m 8 -b 1 ",
                  " > ", paste(postMaker, SppAbbrv, "_uniProtswissProt_blast.out; ", sep = ""), 
                  ...
                  "gzip ", paste(postMaker, SppAbbrv, "_uniProtswissProt_blast.out ", sep = ""),
                  sep = ""))
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix) 
```

####### Over here!!! - need to remove QsubTemps:  
**To remove the output files after you are done:**
```{r}
RemoveQsubTempFiles(sharedPathAn, prefix)
```
...
*** Note: There ar lots of failures going on - see how to set things up properly in the new directory...
```{r}
...
cmd <- with(metadataAssemblies,
            paste("cd ", postMaker, " && ", interproPath, " ",
                  " -i ", paste(postMaker, SppAbbrv, "_rnd3.maker.proteins.renamed.fasta", sep = ""),
...
* Note:  
InterProScan may fail. Make sure to check the error and output log files. Sometimes it's just a matter of restarting a run.
I had a run fail because the protein fasta had a sequence with stop codons (*). In this case, you need to remove all the stop 
codin (*) characters from the fasta file, and then restart the run.  
  
### Add putative gene functions using the output from the BLAST job of our proteins against UniProt/SwissProt.  
We can use maker_functional_gff to add putative functions from the BLAST report to out GFF3 files, and maker_functional_fasta to add 
the putative functions from the BLAST report to out FASTA files. Note that this method only supports UniProt/SwissProt headers.
```{r}
prefix <- "post_maker_func_gff"
cmd    <- with(metadataAssemblies, 
               paste("perl ", makerFuncGffPath, " ", uniProtSwissPDB, " ", 
                     paste(RepeatDBPath, SppAbbrv, "_uniProtswissProt_blast.renamed.out", sep = ""), " ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3", sep = ""), " > ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker_jb.renamed.putative_function.gff", sep = ""),
                     sep = "")) 
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix, node)

prefix2 <- "post_maker_func_fasta_proteins"
cmd2    <- with(metadataAssemblies, 
               paste("perl ", makerFuncFasta, " ", uniProtSwissPDB, " ", 
                     paste(RepeatDBPath, SppAbbrv, "_uniProtswissProt_blast.renamed.out", sep = ""), " ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.fasta", sep = ""), " > ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.proteins.renamed.putative_function.fasta", sep = ""),
                     sep = "")) 
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd2, prefix2, suffix, node)

prefix3 <- "post_maker_func_fasta_transcripts"
cmd3    <- with(metadataAssemblies, 
               paste("perl ", makerFuncFasta, " ", uniProtSwissPDB, " ", 
                     paste(RepeatDBPath, SppAbbrv, "_uniProtswissProt_blast.renamed.out", sep = ""), " ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.transcripts.renamed.fasta", sep = ""), " > ",
                     paste(makerRnd3_OutputPath, SppAbbrv, "_rnd3.all.maker.transcripts.renamed.putative_function.fasta", sep = ""),
                     sep = "")) 
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd3, prefix3, suffix, node)
```
...
### Generation of NCBI Submission Files with GAG.  
