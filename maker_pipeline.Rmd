...
cmd <- with(metadataAssemblies,
            paste("cd ", postMaker, " && ", interproPath, " -mode cluster -clusterrunid ipr_cluster_monitor_job ", 
                  " -i ", paste(postMaker, SppAbbrv, "_rnd3.maker.proteins.renamed.fasta", sep = ""),
                  " -appl ", interproAppsNoGene3d, " -iprlookup -goterms --cpu ", node, sep = ""))
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix, node)
```
...
We can use maker_functional_gff to add putative functions from the BLAST report to our GFF3 files, and maker_functional_fasta to add the putative functions from the BLAST report to out FASTA files. Note that this method only supports UniProt/SwissProt headers.
```{r}
prefix <- "post_maker_func_gff_fasta"
cmd    <- with(metadataAssemblies, 
               paste(# Add BLAST putative functions to the GFF3 file using maker_functional_gff:    
                     "perl ", makerFuncGffPath, " ", uniProtSwissPDB, " ", 
                     paste(postMaker, SppAbbrv, "_uniProtswissProt_blast.Renamed.out", sep = ""), " ",
                     paste(postMaker, SppAbbrv, "_rnd3.all.maker_jb.renamed.gff3", sep = ""), " > ",
                     paste(postMaker, SppAbbrv, "_rnd3.all.maker_jb.renamed.putative_function.gff; ", sep = ""),
                     # Add BLAST putative functions to proteins fasta using maker_functional_fasta:  
                     "perl ", makerFuncFasta, " ", uniProtSwissPDB, " ",
                     paste(postMaker, SppAbbrv, "_uniProtswissProt_blast.Renamed.out", sep = ""), " ",
                     paste(postMaker, SppAbbrv, "_rnd3.maker.proteins.renamed.fasta", sep = ""), " > ",
                     paste(postMaker, SppAbbrv, "_rnd3.maker.proteins.renamed.putative_func.fasta; ", sep = ""),
                     # Add BLAST putative functions to transcripts fasta using maker_functional_fasta:   
                     "perl ", makerFuncFasta, " ", uniProtSwissPDB, " ",
                     paste(postMaker, SppAbbrv, "_uniProtswissProt_blast.Renamed.out", sep = ""), " ",
                     paste(postMaker, SppAbbrv, "_rnd3.maker.transcripts.renamed.fasta", sep = ""), " > ",
                     paste(postMaker, SppAbbrv, "_rnd3.maker.transcripts.renamed.putative_func.fasta; ", sep = ""),
                     # Gzip the renamed only files:
                     "cd ", postMaker, " && gzip *.renamed.fast* ; ", 
                     "cd ", postMaker, " && gzip *.renamed.gff* ",
                     sep = "")) 
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix, node)
```
...
### Add protein domain information to the final annotations using the .tsv report generated from the InterProScan run.      
