...
Now we have the prerequisite data for running MAKER.
...
testing things
```{r}
#replace "genome= " "genome=/home/cfia/fna" -- maker_opts.ctl
x <- "genome= "
y <- "genome=/home/cfia.fna"

filenames <- paste(sharedPathAn, "test_edit_maker_CTL_files/maker_opts.ctl", sep = "")

for(f in filenames){
    x <- readLines(f)
    y <- gsub("genome= ", "genome=/home/cfia.fna ", x)
    cat(y, file=f, sep="\n")
}

# must do changes for genome, and protein in the above command. 
# "protein= ", "protein=/home/CFIA-ACIA/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/References/Botrytis_cinerea.ASM83294v1.pep.all.fa ",
...
cmd <- paste("/opt/rocks/bin/replace ",
             "altest= ", 
             "altest=/home/CFIA-ACIA/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/References/Botrytis_cinerea.ASM83294v1.cdna.all.fa ",
             "altest_gff= ", "altest_gff=/home/CFIA-ACIA/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/References/Botrytis_cinerea.ASM83294v1.37.gff3 ",
             ...
             " -- ", paste(sharedPathAn, "test_edit_maker_CTL_files/maker_opts.ctl", sep = ""),
             sep = "")
...
metadataAssemblies$FixedNewAssemName
paste(metadataAssemblies$RepeatDBPath, "Full_mask/", metadataAssemblies$SppAbbrv, ".assembly.full_mask.complex.reformat.gff3 ", sep = "")
```

Create individual bash scripts in each maker_round_1 directory to qsub the runs:
```{r}
# Options for maker_opts.ctl file:


# how many processors will you use when qloged-in for this bash script?
for(i in 1:nrow(metadataAssemblies)){
    prefix <- "maker_round_1"
    cmd <- paste("maker maker_exe.ctl maker_bopts.ctl maker_opts.ctl ", sep = "")
    sharedPathAn2 <- paste(metadataAssemblies$RepeatDBPath[i], sep = "")
    suffix <- ".sub"; cat(bashDirections); MakeQsubs_test(cmd, prefix, suffix, sharedPathAn2)
}
```
...
Edit the maker_opts.ctl file with gedit so it has the following details:
```{r}
...
altest=/home/CFIA-ACIA/girouxeml/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/Lachnellula_suecica/Lsue_NCBI_files/Lsue_NCBI/Botrytis_cinerea.ASM83294v1.cdna.all.fa #EST/cDNA sequence file in fasta format from an alternate organism
est_gff= #aligned ESTs or mRNA-seq from an external GFF3 file
altest_gff=/home/CFIA-ACIA/girouxeml/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/Lachnellula_suecica/Lsue_NCBI_files/Lsue_NCBI/Botrytis_cinerea.ASM83294v1.37.gff3 #aligned ESTs from a closely relate species in GFF3 format

##-----Protein Homology Evidence (for best results provide a file for at least one)
protein=/home/CFIA-ACIA/girouxeml/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/Lachnellula_suecica/Lsue_NCBI_files/Lsue_NCBI/Botrytis_cinerea.ASM83294v1.pep.all.fa  #protein sequence file in fasta format (i.e. from mutiple oransisms)
protein_gff=  #aligned protein homology evidence from an external GFF3 file

##-----Repeat Masking (leave values blank to skip repeat masking)
model_org=simple #select a model organism for RepBase masking in RepeatMasker
rmlib= #provide an organism specific repeat library in fasta format for RepeatMasker
repeat_protein=/home/CFIA-ACIA/girouxeml/prog/maker/data/te_proteins.fasta #provide a fasta file of transposable element proteins for RepeatRunner
rm_gff=/home/CFIA-ACIA/girouxeml/PIRL_working_directory/Lachnellula_species_GenomeAn_IonTorrent_2017/Lachnellula_suecica/Lsue_RepeatDB/Full_mask/Lsue.assembly.full_mask.complex.reformat.gff3 #pre-identified repeat elements from an external GFF3 file
...
augustus_species= botrytis_cinerea #Augustus gene prediction species model
...
Specifying the GFF3 annotation file for the annotated complex repeats (rm_gff) has the 
effect of hard masking these repeats so that they do not confound our ability to identify 
coding genes. We let MAKER identify simple repeats internally, since it will soft mask these, 
allowing them to be available for gene annotation. This isn't a typical approach but has to be done 
if you want to do more than one succeessive round of RepeatMasker. I verified this would work with 
the MAKER maintainers here. Two other important settings are est2genome and protein2genome, which 
are set to 1 so that MAKER gene predictions are based on the aligned transcripts and proteins (the 
only form of evidence we currently have). I also construct the MAKER command in a Bash script so 
it is easy to run and keep track of.
```{r}
maker maker_opts.ctl maker_bopts.ctl maker_exe.ctl
```

Note  I should try to set-up a qsub bash script, but haven't yet for this. The example I have is:
In a bash script called "round1_run_maker.sh"
Have the following:
...
and the assembly quality (smaller, less contiguous scaffolds == longer). We conclude by assembling 
...
and it will result in nothing being returned. Instead, set the options to â€œ-nâ€ (no filters). 
