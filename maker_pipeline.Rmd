...
Generate the maker control files within each maker_round_1:
```{r}
...
for(k in 1:nrow(metadataAssemblies)){
    dir.create(paste(metadataAssemblies$snapMakerRnd2Path[k], "round2", sep = ""), showWarnings = TRUE, recursive = FALSE)}
metadataAssemblies$snapRnd2MakerRnd2Path <- paste(metadataAssemblies$snapMakerRnd2Path, "round2/", sep = "")

cmd <- with(metadataAssemblies,
            paste("cd ", snapRnd2MakerRnd1Path, " && ", maker2zffPath, " -n -d ", 
                  paste(makerRnd2_OutputPath, SppAbbrv, "_rnd2_master_datastore_index.log", sep = ""), sep = ""))
sapply(cmd, function(x) system(x))
```
...
Take a quick look at number of seqs in genome.dna file
```{r}
cmd <- with(metadataAssemblies, paste("cd ", snapRnd2MakerRnd2Path, " && ", 'grep ">" genome.dna | wc -l ', sep = ""))
sapply(cmd, function(x) system(x))

cmd <- with(metadataAssemblies, file.rename(paste(snapRnd2MakerRnd2Path, "genome.dna", sep = ""),
                                            paste(snapRnd2MakerRnd2Path, SppAbbrv, "_rnd2.zff.dna", sep = "")))
sapply(cmd, function(x) system(x))

cmd <- with(metadataAssemblies, file.rename(paste(snapRnd2MakerRnd2Path, "genome.ann", sep = ""),
                                            paste(snapRnd2MakerRnd2Path, SppAbbrv, "_rnd2.zff.ann", sep = "")))
sapply(cmd, function(x) system(x))
```

Gather some stats and validate:
```{r}
cmd <- with(metadataAssemblies, paste("cd ", snapRnd2MakerRnd2Path, " && ", fathomPath, " ", 
...
cmd <- with(metadataAssemblies, paste("cd ", snapRnd2MakerRnd2Path, " && ",
                  fathomPath, " uni.ann uni.dna -export 1000 -plus > uni-plus.log 2>&1 ", sep = ""))
sapply(cmd, function(x) system(x))
```
