...
# In this file, I make it so that you can set where the sharedPathAn will be so that bash scripts
# can be written to various folders. I call it sharedPathAn2 so as not to over-ride sharedPathAn.

MakeQsubs_test <- function(cmd, prefix, suffix = ".sub", sharedPathAn2, node =1) {
  dir.create(paste(sharedPathAn2, prefix, sep = ""), 
             showWarnings = TRUE, 
             recursive = FALSE)
  outPath <- paste(sharedPathAn2, prefix, "/", sep="")
