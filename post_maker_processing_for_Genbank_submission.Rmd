...
RemoveQsubTempFiles(sharedPathAn, prefix)
```
*** Over here - did things manually to get the egited gff and the genome.fsa with <200bp removed.
...
*** Over here - following paths need to be confirmed/fixed:
...
cmd <- with(metadataAssemblies,
            paste("sed -i 's/>protein|/>/g' ", 
                  paste(postMakerGAG_pass2, SppAbbrv, ".proteins.gagPass2.fasta", sep = ""),
                  sep = ""))
cmd[1]
sapply(cmd, function(x) system(x))
...
*** Over here - Satuday - at CFIA - leaving to work from home - blast for lari not yet completed.
